...
Sortie numérique commutée très vite avec un rapport cyclique variable: analogWrite(pin, 0–255) sur pins PWM (~) (~490/980 Hz selon carte). Donne une moyenne équivalente à une tension analogique (utile pour LED, vitesse moteur, buzzer piezo).
...
Une variable est un espace nommé en mémoire qui stocke une valeur qu’on peut lire et (souvent) modifier pendant l’exécution du programme. Elle a un type (ex. int, float, bool) qui détermine la forme des données et l’espace mémoire utilisé.  
